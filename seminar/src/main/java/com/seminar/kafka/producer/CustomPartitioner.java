...
public class CustomPartitioner implements Partitioner {
    ...
    public int partition(String topic, Object key, byte[] keyBytes, Object value, byte[] valueBytes, Cluster cluster) {
        ...
        int numSpecialPartitions = (int)(numPartitions * 0.5); // 2 * 0.5 = 1 -> (int) 1
        int partitionIndex;
        // Handle Exception
        if (keyBytes == null) {
        ...
        if (((String)key).equals(specialKeyName) ) {
            ...
            partitionIndex = Utils.toPositive(Utils.murmur2(valueBytes)) % numSpecialPartitions; // 0
        }
        else {
            partitionIndex = Utils.toPositive(Utils.murmur2(keyBytes))
                    % (numPartitions - numSpecialPartitions) + numSpecialPartitions; // 1
        }
